...
export async function parseRestaurants(html: string) {
  ...
  let optionsHtml = document.querySelectorAll(
    'select#MainContent_RestaurantDropDownList>option:not([value=""])'
  );
  optionsHtml.forEach((link) => {
    restaurants.push(new Restaurant(link?.attrs?.value, link?.text));
  });
  return restaurants;
}
...
export async function extractFormParams(html: string) {
  ...
  return new Map(
    document
      ...
      .map((e) => [e?.attrs?.name, e?.attrs?.value])
  );
}
...
export async function parse(
  content: any,
  callback: (content: Day[] | undefined, diets: Diet[] | undefined) => void
) {
  ...
  new pdfParser.PdfReader().parseBuffer(
    content,
    (pdfError: { parserError: string }, pdf: any) => {
      if (pdfError) {
          ...
          if (item.length > 0) {
            ...
            if (firstEntry.text.match(dateRegex)) {
              ...
              if (tmpContent.length > 0) {
                tmpItems.push(
                  new Meal(
                    HashUtils.sha1Digest(type + mealType + "_" + tmpContent),
                    tmpContent
                  )
                );
                tmpContent = "";
...
export async function parseRSSFeed(content: any) {
  try {
    if (
      content &&
      content.name === "rss" &&
      content.children.filter((i: any) => {
        return i.name === "channel";
      })
    ) {
      let channel = content.children.filter((i: any) => {
        return i.name === "channel";
      })[0];
      let items = channel.children.filter((i: any) => {
        return i.name === "item";
      });
      let days: Day[] = [];
        ...
        if (title && desc) {
          ...
          for (let subItem of desc.split("<br>")) {
            ...
            tempMenuList.push(
              new Menu(name, [
                new Meal(
                  HashUtils.sha1Digest(type + name + "_" + value),
                  value
                ),
              ])
            );
