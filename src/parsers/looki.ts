      ...
      if (boxContent.match(dateRegex)) {
            ...
            new Day(currentDate, [
              {
                ...
                meals: [
                  new Meal(
                    HashUtils.sha1Digest(type + currentBasicContent),
                    currentBasicContent
                  ),
                ],
              },
        ...
        if (boxContent.match(groupRegex)) {
          ...
          boxContent.split("\n").forEach((line) => {
            ...
            if (line.endsWith(":")) {
              if (tempContent.length > 0 && currentType !== undefined) {
                tempMenus.push(
                  new Menu(currentType, [
                    new Meal(
                      HashUtils.sha1Digest(type + tempContent),
                      tempContent
                    ),
                  ])
                );
              ...
              if (regExp !== null) currentType = regExp[1];
          ...
          if (tempContent.length > 0 && currentType !== undefined) {
            tempMenus.push(
              new Menu(currentType, [
                new Meal(HashUtils.sha1Digest(type + tempContent), tempContent),
              ])
            );
            tempContent = "";
          }
