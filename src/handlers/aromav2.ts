...
import {
  extractFormParams,
  extractRestaurantPDFLink,
  parse,
  parseRestaurants,
  parseRSSFeed,
} from "../parsers/aromiv2";
...
const getSeleniumDriver = () => {
  // The TypeScript typings for selenium-webdriver's chrome.Options in the
  // version we use don't expose helper methods like headless()/windowSize(),
  // even though they exist at runtime. Cast to any so the compiler is happy.
  let options: any = new Options();
  options = options.headless().windowSize({ width: 1270, height: 700 });
...
async function selectRestaurant(driver: ThenableWebDriver, id: string) {
  ...
  if (position === -1) throw new Error("Restaurant not found, check the ID");
...
async function getRestaurantPDFLink(driver: ThenableWebDriver) {
  ...
  if (href == null) {
    ...
    await driver.wait(() => {
      return driver
        .findElement(By.id("MainContent_PdfUrl"))
        .getAttribute("href")
        .then((href) => {
          return href != null;
        });
...
async function getRestaurantPDFLinkDirect(url: string, restaurantId: string) {
  const httpResp: { body: string; cookies: Cookies | undefined } =
  ...
  const data = await fetch(url, {
    headers: {
      "User-Agent":
        "Mozilla/5.0 (Haistka; Pska:) Saatana/20011109 CGIDevausTiimilleVitutJaPotkut/69.0",
      Accept: "*/*",
      ...
      Pragma: "no-cache",
      Cookie: Object.keys(httpResp.cookies || {})
        .map((i) => `${i}=${(httpResp.cookies || {})[i]}`)
        .join("; "),
    },
...
export async function getRestaurantPage(req: Request, res: Response) {
  ...
  const fetchDocument = async (
    ...
    const contains = (item: string, items: Diet[]) => {
      let found = false;
      items.forEach((item2) => {
        if (item.toLowerCase() == item2.name.toLowerCase()) found = true;
      });
      return found;
    };
    ...
    if (
      ...
      diets1.forEach((dItem) => {
        if (!contains(dItem.name, diets)) {
          diets.push(dItem);
        }
      });
